...
import { Globe, Trophy, ChevronLeft, ChevronRight, Star, Flame, RotateCcw, TrendingUp, MapPin, Medal, Send, List, Check } from 'lucide-react';
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 sm:p-6">
      ...
      <div className="w-full max-w-2xl">
        ...
        {gameState === 'start' && (
          <div className="card p-8 animate-bounce-in">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center mb-8">
              ...
              <button onClick={() => selectMode('endless')} className="text-lg flex items-center justify-center gap-2 bg-purple-700 hover:bg-purple-600 active:bg-purple-800 text-white font-semibold py-3 px-6 rounded-xl transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
            ...
            <div className="pt-6 border-t border-slate-700">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <h4 className="text-sm font-semibold text-purple-400 flex items-center gap-1 mb-2"><Flame className="w-4 h-4" />Endless</h4>
                  {endlessHighScores.length === 0 ? <p className="text-slate-500 text-xs">No scores yet</p> : (
                    <div className="space-y-2">{endlessHighScores.map((hs, i) => (
                      <div key={i} className="flex items-center justify-between p-2 rounded-lg bg-slate-700/50">
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex items-center gap-1"><Flame className="w-3 h-3 text-purple-400" /><span className="font-semibold text-sm text-purple-400">{hs.streak}</span></div>
                          {hs.difficulty && <span className={diffDot(hs.difficulty)} title={hs.difficulty} />}
                        </div>
                      </div>
                    ))}</div>
            ...
            <div className="mt-6 pt-6 border-t border-slate-700">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-1">
                  ...
                  <button onClick={() => { setHomeLeaderboardMode('endless'); setHomePage(0); }} className={`text-xs font-semibold px-3 py-1 rounded-lg transition-colors ${homeLeaderboardMode === 'endless' ? 'bg-purple-600 text-white' : 'bg-slate-700 text-slate-400 hover:bg-slate-600'}`}>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div className="text-right">
                                {homeLeaderboardMode === 'classic'
                                  ...
                                  : <><span className="font-bold text-sm text-purple-400 flex items-center gap-1"><Flame className="w-3 h-3" />{entry.streak}</span><span className="text-xs text-slate-400 ml-1">({entry.weighted_score}pts)</span></>}
                              </div>
                            </div>
                          </div>
                        );
          ...
          <>
            ...
            <div className="flex items-center justify-between mb-6 animate-slide-up">
              ...
              <div className="text-sm text-slate-400">
                {gameMode === 'endless'
                  ? <span className="text-purple-400 font-semibold flex items-center gap-1"><Flame className="w-4 h-4" />Endless</span>
        ...
        {gameState === 'finished' && gameMode === 'endless' && (
          <div className="card p-8 text-center animate-bounce-in">
            <div className="mb-6">
              <div className="inline-flex items-center justify-center w-24 h-24 rounded-full mb-4 bg-gradient-to-br from-purple-500 to-purple-700">
              ...
              <div className="inline-flex items-center gap-3 px-6 py-3 rounded-2xl bg-purple-500/20 border border-purple-500/30">
                <Flame className="w-6 h-6 text-purple-400" />
                <span className="text-slate-300">Streak:</span>
                <span className="font-bold text-3xl text-purple-400">{bestStreak}</span>
              </div>
            </div>
            <div className="mt-6 pt-6 border-t border-slate-700">
              {!nameSubmitted ? (
                <div>
                  ...
                  <div className="flex gap-2">
                    <input value={playerName} onChange={e => setPlayerName(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSubmitScore()}
                      placeholder="Your name..." maxLength={20}
                      className="flex-1 bg-slate-700 border border-slate-600 rounded-xl px-4 py-2 text-white placeholder-slate-400 focus:outline-none focus:border-purple-400" />
                    <button onClick={handleSubmitScore} disabled={submitting || !playerName.trim()}
                      className="bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-white font-semibold px-4 py-2 rounded-xl flex items-center gap-2 transition-colors">
                      {submitting ? '...' : <Send className="w-4 h-4" />}
                    </button>
                  </div>
                  {submitError && <p className="text-red-400 text-xs mt-2">Failed to submit — check your connection.</p>}
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center mt-6">
              <button onClick={() => { setPendingMode('endless'); setNameSubmitted(false); setPlayerName(''); startGame(difficulty); }} className="bg-purple-700 hover:bg-purple-600 text-white font-semibold py-3 px-6 rounded-xl transition-all duration-200 shadow-lg flex items-center justify-center gap-2">
          ...
          <div className="card p-8 animate-bounce-in">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => { setLeaderboardMode('endless'); openLeaderboard('endless'); }} className={`text-sm font-semibold px-4 py-2 rounded-xl transition-colors ${leaderboardMode === 'endless' ? 'bg-purple-600 text-white' : 'bg-slate-700 text-slate-400 hover:bg-slate-600'}`}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="text-right">
                        {leaderboardMode === 'classic'
                          ...
                          : <><span className="font-bold text-purple-400 flex items-center gap-1"><Flame className="w-3 h-3" />{entry.streak}</span><span className="text-xs text-slate-400 ml-1">({entry.weighted_score}pts)</span></>}
                      </div>
                    </div>
                  </div>
                ))}
